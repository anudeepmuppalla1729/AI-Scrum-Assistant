...
const NAV_ITEMS: NavItem[] = [
  ...
  { path: "/sprints", label: "Sprints", icon: ClipboardList, requiresAuth: true },
  { path: "/prd", label: "Backlog Generator", icon: FileText, requiresAuth: false },
  { path: "/chat", label: "Chat", icon: MessageSquare, requiresAuth: true },
  { path: "/documents", label: "Documents", icon: FolderKanban, requiresAuth: true },
  { path: "/docs", label: "Docs", icon: BookOpen, requiresAuth: false },
];
  ...
  return (
    <header className="topbar">
      <div className="topbar-inner">
        <button className="topbar-brand" onClick={() => navigate(isAuthenticated ? "/workspace" : "/")}>
          ...
          <span className="topbar-brand-text">Parallel Agile Assistant</span>
        </button>
        ...
        <nav className="topbar-nav">
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <button
                key={item.path}
                className={`topbar-nav-item ${isActive(item.path) ? "active" : ""} ${locked ? "locked" : ""}`}
                onClick={() => handleNavClick(item)}
                title={locked ? "Login required" : undefined}
              >
                <item.icon size={16} />
                <span>{item.label}</span>
                {locked && <Lock size={10} className="topbar-lock-icon" />}
              </button>
            );
          })}
        </nav>
        ...
        <div className="topbar-right">
          {isAuthenticated ? (
            ...
            <button className="topbar-nav-item" onClick={() => navigate("/login")}>
              Login
            </button>
          )}
        </div>
