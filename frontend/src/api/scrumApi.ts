...
export const uploadPRD = async (
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to upload PRD: ${response.statusText} - ${errorText}`,
    );
  }
...
export const pushSuggestionsToJira = async (
  payload: PushToJiraRequest,
): Promise<PushToJiraResponse> => {
...
export const createPRDSession = async (
  data: CreatePRDSessionRequest,
): Promise<PRDSession> => {
...
export const updatePRDSession = async (
  sessionId: string,
  data: UpdatePRDSessionRequest,
): Promise<PRDSession> => {
  const response = await fetch(`${API_BASE_URL}/prd/session/${sessionId}`, {
    method: "PATCH",
    headers: {
